...
void pcap_callback(u_char* user, const struct pcap_pkthdr *h, const u_char * bytes){
    ...
    if( ntohs(eptr-> ether_type) == ETHERTYPE_IP ){
        ...
        if( ipptr->protocol==IPPROTO_TCP ){
            printf("now is tcp protocol\n");
            struct tcphdr* tcpptr = ( struct tcphdr *)(bytes + sizeof( struct ether_header )+
                    sizeof( struct iphdr ) );
            u_char status=((tcpptr->syn)<<0)|((tcpptr->ack)<<1)|((tcpptr->fin)<<2);
            printf("status = %u\n",status);
        }
        else printf("now is not tcp protocol\n");
    }
    else printf("Now is not IP\n");
    scanf("%s",ts);
}
...
int main(int argc,char *argv[]){
    ...
    if(pcap_compile( handle , &filter , filter_rgx , 1 , 0 )!=0){
        printf(" compile regluar expression error \n");
        return 0;
    }

    // set the regular expression
    if(pcap_setfilter(handle,&filter)!=0){
        printf(" set regular expression error \n");
        return 0;
    }
